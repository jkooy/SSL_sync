...
parser.add_argument('--data',  default='../SUN397', type=str,
...
parser.add_argument('--save-path',default='SUN397_save/10MoCo_sun',type=str)
# moco specific configs:
parser.add_argument('--moco-dim', default=128, type=int,
                    help='feature dimension (default: 128)')
parser.add_argument('--moco-k', default=6528, type=int,
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    logger.info('len %i', origin_len)  
    train_dataset, test_dataset = torch.utils.data.random_split(train_dataset, [int(0.1*len(train_dataset)), origin_len-int(0.1*len(train_dataset))])
    logger.info('len train %i', len(train_dataset))
...
#         if epoch!=0 and epoch % args.save_epoch == 0:
            ...
            filename = os.path.join(args.save_path,'checkpoint_epoch_{:04d}.pth.tar'.format(epoch))
